...
# dataset -> dataloader -> model -> train -> test
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--prompt_class_name", type=int, default=0, help="Prompt contain class name?"
    )
    parser.add_argument(
        "--lr_num_cycles",
        type=int,
...
class DreamBoothDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if args.prompt_class_name:
            file_name = os.path.basename(path)
            class_name = os.path.splitext(file_name)[0]
            prompt = class_name+' in '+ self.instance_prompt
        else:
...
def main(args):
    ...
    # peft_model = get_peft_model(unet, unet_lora_config)
    # peft_model.print_trainable_parameters()
    # exit()
    # Now we will add new LoRA weights to the text encoder
    # param_info = "Model's learnable parameters:\n"
    # for name, param in unet.named_parameters():
    #      param_info += f"Parameter name: {name}, shape: {param.shape}\n"
    # with open("files/model_parameters.txt", "w") as file:
    #     file.write(param_info)
    # exit()
    # Optimizer creation
    optimizer = AdamW(
...
if __name__ == "__main__":
    args = parse_args()
    print(args)
    print('class_data_dir:',args.class_data_dir)
    main(args)
